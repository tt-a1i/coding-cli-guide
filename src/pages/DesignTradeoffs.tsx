...
`} />
 ...
 <th className="p-3 text-left text-heading">原因</th>
 </tr>
 </thead>
 <tbody>
 ...
 Gemini CLI 最显著的战略选择是<strong className="text-heading">拒绝纯并行</strong>，
 转而采用<strong className="text-heading">顺序请求队列</strong>。
...
function SafetyTab() {
 ...
 <p className="text-body mb-4">
 三层审批模式体现了<strong className="text-heading">安全优先</strong>与<strong className="text-heading">效率需求</strong>的平衡：
 </p>
...
function validateApprovalMode(mode: ApprovalMode, cwd: string): ApprovalMode {
 ...
 <p className="text-sm">
 增加了代码复杂度，但<strong className="text-heading">显著提升安全性</strong>。
 在不同信任上下文中提供差异化的用户体验。
 </p>
 </HighlightBox>
 </Layer>
 ...
 <p className="text-body mb-4">
 上下文压缩需要选择<strong className="text-heading">语义安全</strong>的分割点：
 </p>
...
function PerformanceTab() {
 ...
 <Layer title="⚡ 并行文件发现">
 <p className="text-body mb-4">
 BFS 文件搜索采用<strong className="text-heading">批量并行</strong>策略：
 </p>
...
class ModelConfigCache {
 ...
 <div className="text-heading text-xl font-bold">5 min</div>
 <div className="text-dim text-xs">TTL 时长</div>
 </div>
 <div className="p-3 bg-elevated rounded-lg text-center">
 <div className="text-heading text-xl font-bold">单例</div>
...
class LoopDetectionService {
 ...
 <div className="text-heading font-semibold mb-2">Layer 1: 工具重复</div>
...
`} />
 ...
 <ul className="text-body text-sm space-y-1 list-disc list-inside">
 <li><strong className="text-heading">队列模式</strong>：处理异步到达的数据流</li>
 <li><strong className="text-heading">单例遥测</strong>：确保全局数据一致性</li>
 <li><strong className="text-heading">顺序执行</strong>：避免状态竞争条件</li>
 <li><strong className="text-heading">优雅降级</strong>：多层回退保证可用性</li>
 </ul>
 </HighlightBox>
 </div>
 );
...
function AlternativesTab() {
 ...
 <tr>
 <td className="p-3 text-body">PTY 多层回退</td>
 <td className="p-3 text-heading text-xs">
 • 100% 环境兼容<br/>
 • 优雅降级
 </td>
...
`} />
 ...
 <div>
 <div className="text-heading font-medium">1. 有限并行执行</div>
 <p className="text-body">
 如果 AI 模型能够可靠地标注工具调用之间的依赖关系，可以考虑对无依赖的工具实现并行执行。
 <span className="text-dim">（需要模型能力提升）</span>
 </p>
 </div>
 <div>
 <div className="text-heading font-medium">2. 更细粒度的权限</div>
 <p className="text-body">
 目前的审批模式是全局的，未来可能支持按工具、按目录的细粒度权限控制。
 <span className="text-dim">（需要用户需求验证）</span>
 </p>
 </div>
 <div>
 <div className="text-heading font-medium">3. 分布式状态</div>
 <p className="text-body">
 当前的单例遥测和队列模式假设单进程运行，如果需要支持分布式场景，需要重新设计。
 <span className="text-dim">（取决于产品方向）</span>
 </p>
 </div>
 </div>
 </HighlightBox>
