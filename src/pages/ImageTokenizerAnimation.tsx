...
export default function ImageTokenizerAnimation() {
 ...
 <h1 className="text-2xl font-bold mb-2 text-heading">Image Tokenizer 二进制解析</h1>
 ...
 <h3 className="text-sm font-semibold text-body mb-3">支持的图像格式签名</h3>
 <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
 {Object.entries(FORMAT_SIGNATURES).map(([key, sig]) => (
 <div key={key} className="p-2 bg-surface rounded">
 <span className="font-medium text-heading">{sig.name}</span>
 <div className="text-dim font-mono mt-1">{sig.hex}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 onClick={reset}
 disabled={phase === 'idle'}
 className="px-4 py-2 bg-elevated hover:bg-elevated disabled:bg-surface disabled:cursor-not-allowed rounded text-heading text-sm transition-colors"
 >
 重置
 </button>
 </div>
 ...
 <span className={`text-xs ${phase === p ? 'text-heading' : 'text-dim'}`}>
 {p === 'loading' ? '加载' : p === 'detecting' ? '检测' : p === 'parsing' ? '解析' : p === 'scaling' ? '缩放' : '完成'}
 </span>
 {idx < 4 && <div className="w-8 h-0.5 bg-elevated" />}
 </div>
 ))}
 </div>
 ...
 <div className="font-mono text-xs bg-base/50 p-3 rounded overflow-x-auto">
 ...
 <div className="mt-2 text-dim flex flex-wrap gap-1">
 ...
 {/* Parse Info */}
 {parseInfo && (
 <div className="mt-3 p-3 bg-elevated/30 border border-edge rounded">
 <div className="text-xs text-heading">{parseInfo}</div>
 </div>
 )}

 {/* Parsed Dimensions */}
 {parsedDimensions && (
 <div className="mt-3 p-3 bg-elevated border border-edge rounded">
 <div className="flex items-center gap-4">
 <div>
 <span className="text-heading text-xs">宽度:</span>
 <span className="ml-2 font-bold text-heading">{parsedDimensions.width}px</span>
 </div>
 <div>
 <span className="text-heading text-xs">高度:</span>
 <span className="ml-2 font-bold text-heading">{parsedDimensions.height}px</span>
 </div>
 <div>
 <span className="text-heading text-xs">像素总数:</span>
 ...
 <div className="text-dim">每 Token 像素</div>
 <div className="font-mono text-body">28×28 = {PIXELS_PER_TOKEN}</div>
 </div>
 <div className="p-2 bg-surface rounded text-center">
 <div className="text-dim">最小 Token</div>
 <div className="font-mono text-body">{MIN_TOKENS}</div>
 </div>
 <div className="p-2 bg-surface rounded text-center">
 <div className="text-dim">最大 Token</div>
 <div className="font-mono text-body">{MAX_TOKENS.toLocaleString()}</div>
 </div>
 </div>
 ...
 尺寸: <span className="text-heading">{step.wBar}×{step.hBar}</span>
 </div>
 <div className="text-body">
 像素: <span className="text-heading">{step.pixels.toLocaleString()}</span>
 </div>
 </div>
 {step.action && (
 <div className="text-xs text-dim font-mono bg-base/30 p-2 rounded">
 {step.action}
 </div>
 )}
 ...
 <h3 className="text-sm font-semibold text-body mb-4">算法流程图</h3>
 <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
 <div className={`p-2 rounded border ${phase === 'loading' ? ' border-edge bg-elevated/30' : ' border-edge'}`}>
 加载二进制
 </div>
 ...
 <span className="text-dim">→</span>
 <div className={`p-2 rounded border ${phase === 'parsing' ? ' border-edge bg-elevated' : ' border-edge'}`}>
 维度提取
 </div>
 <span className="text-dim">→</span>
 ...
 <h4 className="text-sm font-semibold text-body mb-2">缩小公式 (超出最大)</h4>
 <div className="font-mono text-xs text-body space-y-1">
 ...
 <div className="p-4 bg-surface rounded-lg">
 <h4 className="text-sm font-semibold text-body mb-2">放大公式 (低于最小)</h4>
 <div className="font-mono text-xs text-body space-y-1">
