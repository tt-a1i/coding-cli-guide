...
function Introduction({ isExpanded, onToggle }: { isExpanded: boolean; onToggle: () => void }) {
 return (
 <div className="mb-8 bg-surface/50 rounded-lg border border-edge overflow-hidden">
 <button
 onClick={onToggle}
 className="w-full px-6 py-4 flex items-center justify-between hover:bg-surface transition-colors"
 >
 <div className="flex items-center gap-3">
 <span className="text-xl font-bold text-heading">架构设计理念</span>
 </div>
 ...
 <p><strong className="text-heading">可扩展性</strong>：通过 MCP 协议支持动态工具注册，无需修改核心代码</p>
 </div>
 </div>

 <div className="bg-base rounded-lg p-4 border-l-[3px] border-edge">
 <h4 className="text-heading font-semibold mb-2">分层架构</h4>
 <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-2">
 <div className="bg-surface p-3 rounded border border-edge text-center">
 <div className="text-heading font-semibold text-sm">UI Layer</div>
 <div className="text-xs text-dim mt-1">React/Ink<br/>用户交互</div>
 </div>
 <div className="bg-surface p-3 rounded border border-edge text-center">
 <div className="text-heading font-semibold text-sm">Core Layer</div>
 <div className="text-xs text-dim mt-1">GeminiChat<br/>核心循环</div>
 </div>
 <div className="bg-surface p-3 rounded border border-edge text-center">
 <div className="text-heading font-semibold text-sm">Tool Layer</div>
 <div className="text-xs text-dim mt-1">14 内置工具<br/>+ MCP 扩展</div>
 </div>
 <div className="bg-surface p-3 rounded border border-edge text-center">
 <div className="text-heading font-semibold text-sm">API Layer</div>
 <div className="text-xs text-dim mt-1">Google GenAI<br/>+ Code Assist</div>
 </div>
 </div>
 </div>
 ...
 <p><strong className="text-heading">通用 vs 专用</strong>：支持多 AI 厂商需要抽象层，但增加了维护成本</p>
 </div>
 </div>
 </div>
 )}
...
export function Overview() {
 ...
 <div>
 {/* 版本提示 */}
 <div className="mb-6 px-4 py-3 bg-surface/50 rounded-lg border border-edge flex items-center justify-between">
 <div className="flex items-center gap-3">
 <div>
 <span className="text-body text-sm">本文档基于 </span>
 <code className="px-2 py-0.5 bg-elevated text-heading rounded text-sm font-mono">
...
`} />
 ...
 <h4 className="text-lg font-semibold text-heading">数据流的关键节点</h4>

 <div className="overflow-x-auto">
 <table className="w-full text-sm border border-edge rounded-lg overflow-hidden">
 <thead className="bg-surface">
 <tr>
 <th className="px-4 py-2 text-left text-heading">节点</th>
 <th className="px-4 py-2 text-left text-heading">输入</th>
 <th className="px-4 py-2 text-left text-heading">处理</th>
 <th className="px-4 py-2 text-left text-heading">输出</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-edge">
 <tr className="bg-base/50">
 <td className="px-4 py-2 text-heading font-semibold">UI Layer</td>
 <td className="px-4 py-2 text-heading">用户按键事件</td>
 <td className="px-4 py-2 text-body">解析输入、处理快捷键、渲染 UI</td>
 <td className="px-4 py-2 text-heading">结构化用户消息</td>
 </tr>
 <tr className="bg-base/30">
 <td className="px-4 py-2 text-heading font-semibold">Core Layer</td>
 <td className="px-4 py-2 text-heading">用户消息 + 历史</td>
 <td className="px-4 py-2 text-body">构建 API 请求、管理上下文、处理流</td>
 <td className="px-4 py-2 text-heading">API 请求 / 工具调用指令</td>
 </tr>
 <tr className="bg-base/50">
 <td className="px-4 py-2 text-heading font-semibold">Hook System</td>
 <td className="px-4 py-2 text-heading">事件触发 + 上下文</td>
 <td className="px-4 py-2 text-body">执行钩子脚本、注入环境变量</td>
 <td className="px-4 py-2 text-heading">修改后的上下文 / 环境变量</td>
 </tr>
 <tr className="bg-base/30">
 <td className="px-4 py-2 text-heading font-semibold">Policy Engine</td>
 <td className="px-4 py-2 text-heading">工具调用请求</td>
 <td className="px-4 py-2 text-body">规则匹配、安全检查、权限决策</td>
 <td className="px-4 py-2 text-heading">ALLOW / DENY / ASK_USER</td>
 </tr>
 <tr className="bg-base/50">
 <td className="px-4 py-2 text-heading font-semibold">Tool Layer</td>
 <td className="px-4 py-2 text-heading">工具名 + 参数</td>
 <td className="px-4 py-2 text-body">验证参数、执行操作</td>
 <td className="px-4 py-2 text-heading">执行结果 / 错误信息</td>
 </tr>
 <tr className="bg-base/30">
 <td className="px-4 py-2 text-heading font-semibold">API Layer</td>
 <td className="px-4 py-2 text-heading">HTTP 请求体</td>
 <td className="px-4 py-2 text-body">AI 推理（云端黑盒）</td>
 <td className="px-4 py-2 text-heading">流式响应（chunks/events）</td>
 </tr>
 </tbody>
 </table>
 </div>
 ...
 <HighlightBox title="流式处理的意义" variant="tip">
 <p className="mb-2">
 为什么要用流式而不是等完整响应？
 </p>
 ...
 '什么是 Continuation？',
 <span key="6">
 当流中出现 <code className="bg-surface px-1 rounded">ToolCallRequest</code> 时，
 CLI 执行工具并将 <code className="bg-surface px-1 rounded">functionResponse</code> 作为 continuation 回注给 AI <strong>继续对话</strong>
 <span className="text-dim">（fork-only：OpenAI 兼容层可能用 finish_reason/tool_calls 表达同样语义）</span>
 </span>,
 ],
 ]}
 />
 ...
 <div className="text-xs text-dim mt-1">内置工具</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">100</div>
 <div className="text-xs text-dim mt-1">最大轮次</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">4</div>
 <div className="text-xs text-dim mt-1">审批模式</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">5</div>
 <div className="text-xs text-dim mt-1">认证方式</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">2M</div>
 <div className="text-xs text-dim mt-1">最大 Token</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">∞</div>
 <div className="text-xs text-dim mt-1">MCP 扩展</div>
 </div>
 </div>

 {/* 事件驱动架构数字 */}
 <div className="mt-6">
 <h4 className="text-sm font-semibold text-body mb-3">事件驱动架构</h4>
 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">11</div>
 <div className="text-xs text-dim mt-1">Hook 事件类型</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">3</div>
 <div className="text-xs text-dim mt-1">Policy 决策类型</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">9</div>
 <div className="text-xs text-dim mt-1">消息总线类型</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">4</div>
 <div className="text-xs text-dim mt-1">路由策略</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">2</div>
 <div className="text-xs text-dim mt-1">Agent 类型</div>
 </div>
 <div className="bg-surface rounded-lg p-4 border border-edge text-center">
 <div className="text-2xl font-bold text-heading">3</div>
 <div className="text-xs text-dim mt-1">配置层级</div>
 </div>
 </div>
 </div>
 </Layer>
 ...
 <h4 className="text-lg font-semibold text-heading">UI Layer（CLI 层）</h4>
 <span className="text-xs px-2 py-0.5 rounded bg-elevated text-heading border border-edge/20">UI</span>
 </div>
 <div className="grid md:grid-cols-2 gap-4">
 <div>
 <p className="text-heading mb-2"><strong>解决的问题：</strong></p>
 ...
 <div>
 <p className="text-heading mb-2"><strong>为什么独立？</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
 ...
 <div className="bg-surface rounded-lg p-5 border border-edge">
 <div className="flex items-center gap-3 mb-3">
 <h4 className="text-lg font-semibold text-heading">Core Layer（核心层）</h4>
 <span className="text-xs px-2 py-0.5 rounded bg-elevated text-heading border border-edge/20">Core</span>
 </div>
 <div className="grid md:grid-cols-2 gap-4">
 <div>
 <p className="text-heading mb-2"><strong>解决的问题：</strong></p>
 ...
 <div className="bg-surface rounded-lg p-5 border border-edge">
 <div className="flex items-center gap-3 mb-3">
 <h4 className="text-lg font-semibold text-heading">Tool Layer（工具层）</h4>
 <span className="text-xs px-2 py-0.5 rounded bg-elevated text-heading border border-edge/20">Tool</span>
 </div>
 <div className="grid md:grid-cols-2 gap-4">
 <div>
 <p className="text-heading mb-2"><strong>解决的问题：</strong></p>
 ...
 <HighlightBox title="分层的代价" variant="warning">
...
`} />
 ...
 <div className="bg-surface rounded-lg p-5 border border-edge">
 <div className="flex items-center gap-3 mb-3">
 <h4 className="text-lg font-semibold text-heading">Hook System</h4>
 <span className="text-xs px-2 py-0.5 rounded bg-elevated text-heading border border-edge/20">事件拦截</span>
 </div>
 <div className="grid md:grid-cols-2 gap-4">
 <div>
 <p className="text-heading mb-2"><strong>核心能力：</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
 ...
 <div>
 <p className="text-heading mb-2"><strong>典型场景：</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
 <li>工具执行前注入环境变量</li>
 <li>模型调用后记录日志</li>
 <li>会话开始时加载配置</li>
 ...
 <div>
 <p className="text-heading mb-2"><strong>规则来源：</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
 <li>TOML 配置文件定义规则</li>
 ...
 <div className="bg-surface rounded-lg p-5 border border-edge">
 <div className="flex items-center gap-3 mb-3">
 <h4 className="text-lg font-semibold text-heading">MessageBus</h4>
 <span className="text-xs px-2 py-0.5 rounded bg-elevated text-heading border border-edge/20">发布/订阅</span>
 </div>
 <div className="grid md:grid-cols-2 gap-4">
 <div>
 <p className="text-heading mb-2"><strong>消息类型：</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
 ...
 <div>
 <p className="text-heading mb-2"><strong>设计优势：</strong></p>
 <ul className="pl-5 list-disc text-sm text-body space-y-1">
...
`} />
 ...
 <div>
 <p className="text-body mb-1"><strong>症状：</strong></p>
 <p className="text-dim">请求超时、Connection refused、SSL 错误</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>原因：</strong></p>
 <p className="text-dim">网络问题、API Key 无效、服务商宕机</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>应对：</strong></p>
 <p className="text-dim">检查网络、验证 Key、查看服务商状态页</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-body mb-1"><strong>症状：</strong></p>
 <p className="text-dim">400 错误、"context length exceeded"</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>原因：</strong></p>
 <p className="text-dim">对话太长、文件内容太大、工具结果过多</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>应对：</strong></p>
 <p className="text-dim">压缩历史、截断大文件、开启 /compact</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-start gap-3">
 <div className="flex-1">
 <h4 className="font-semibold text-heading mb-2">工具执行失败</h4>
 <div className="grid md:grid-cols-3 gap-4 text-sm">
 <div>
 <p className="text-body mb-1"><strong>症状：</strong></p>
 <p className="text-dim">ENOENT、Permission denied、Command not found</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>原因：</strong></p>
 <p className="text-dim">路径错误、权限不足、命令不存在</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>应对：</strong></p>
 <p className="text-dim">检查路径、调整权限、安装依赖</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div className="flex items-start gap-3">
 <div className="flex-1">
 <h4 className="font-semibold text-heading mb-2">AI 进入死循环</h4>
 <div className="grid md:grid-cols-3 gap-4 text-sm">
 <div>
 <p className="text-body mb-1"><strong>症状：</strong></p>
 <p className="text-dim">反复调用同一工具、无限重试</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>原因：</strong></p>
 <p className="text-dim">工具结果不清晰、AI 不理解错误信息</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>应对：</strong></p>
 <p className="text-dim">按 Ctrl+C 中断、优化工具返回格式</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <div>
 <p className="text-body mb-1"><strong>症状：</strong></p>
 <p className="text-dim">等待时间长、首 token 延迟高</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>原因：</strong></p>
 <p className="text-dim">上下文太长、服务器负载高、网络延迟</p>
 </div>
 <div>
 <p className="text-body mb-1"><strong>应对：</strong></p>
 <p className="text-dim">压缩上下文、选择更快的模型、使用代理</p>
 </div>
 </div>
 </div>
 </div>
 ...
 <HighlightBox title="故障恢复决策树" variant="info">
 <MermaidDiagram chart={`
flowchart TD
 A[出现问题] --> B{能看到错误信息吗？}
...
`} />
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">工具开发者</h4>
 <p className="text-sm text-body mb-3">想要开发自定义工具</p>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">AI 交互理解</h4>
 <p className="text-sm text-body mb-3">想要理解 AI 如何工作</p>
 <ul className="text-xs text-dim space-y-1">
 <li>→ AI 工具交互机制</li>
 <li>→ Token 生命周期</li>
 <li>→ 会话持久化</li>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">问题排查</h4>
 <p className="text-sm text-body mb-3">想要快速定位问题</p>
 <ul className="text-xs text-dim space-y-1">
 <li>→ 错误恢复决策树</li>
 <li>→ 错误处理机制</li>
 <li>→ 调试技巧</li>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">系统配置</h4>
 <p className="text-sm text-body mb-3">想要定制 CLI 行为</p>
 <ul className="text-xs text-dim space-y-1">
 <li>→ 配置系统</li>
 <li>→ 斜杠命令</li>
 <li>→ 主题定制</li>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">安全专家</h4>
 <p className="text-sm text-body mb-3">想要理解安全机制</p>
 <ul className="text-xs text-dim space-y-1">
 <li>→ 权限与审批</li>
 <li>→ 沙箱执行</li>
 <li>→ 安全边界</li>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge hover:border-edge transition-colors">
 <h4 className="text-heading font-semibold mb-2">扩展开发</h4>
 <p className="text-sm text-body mb-3">想要扩展 CLI 能力</p>
 <ul className="text-xs text-dim space-y-1">
 <li>→ 扩展系统</li>
 <li>→ IDE 集成</li>
 <li>→ MCP 服务器</li>
