        ...
        <div className="px-6 pb-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--terminal-green)]">PTY</div>
              <div className="text-xs text-[var(--text-muted)]">伪终端</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--cyber-blue)]">Bi-Dir</div>
              <div className="text-xs text-[var(--text-muted)]">双向通信</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--amber)]">Resize</div>
              <div className="text-xs text-[var(--text-muted)]">动态调整</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--purple)]">Sandbox</div>
              <div className="text-xs text-[var(--text-muted)]">安全沙箱</div>
            </div>
          </div>

          <div>
            <h4 className="text-sm font-semibold text-[var(--text-muted)] mb-2">核心能力</h4>
...
export function InteractiveShell() {
  ...
  const architectureDiagram = `flowchart TB
    ...
    style Agent fill:#1a1a2e,stroke:#00d4ff
    style PTYSystem fill:#1a1a2e,stroke:#00ff88
    style OS fill:#2d1f3d,stroke:#a855f7`;
...
export function buildSandboxedEnv(
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-2">交互式 Shell (PTY)</h1>
      ...
      <Layer title="概述" icon="📖" defaultOpen={true}>
        <div className="space-y-4">
          <p className="text-[var(--text-secondary)]">
            伪终端 (Pseudo Terminal, PTY) 是 Gemini CLI 中实现<strong className="text-[var(--text-primary)]">双向终端通信</strong>的核心机制。
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--terminal-green)]/30">
              <div className="text-[var(--terminal-green)] font-bold mb-2">普通 Shell 执行</div>
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--cyber-blue)]/30">
              <div className="text-[var(--cyber-blue)] font-bold mb-2">PTY 交互式 Shell</div>
              <ul className="text-sm text-[var(--text-secondary)] space-y-1">
                <li>- 双向实时通信</li>
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--amber)]/30">
              <div className="text-[var(--amber)] font-bold mb-2">关键差异</div>
              <ul className="text-sm text-[var(--text-secondary)] space-y-1">
      ...
      <Layer title="架构设计" icon="🏗️" defaultOpen={true}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--terminal-green)] font-bold mb-2">PtyManager</h4>
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--cyber-blue)] font-bold mb-2">TerminalSession</h4>
            <p className="text-sm text-[var(--text-secondary)]">
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--amber)] font-bold mb-2">InputHandler</h4>
            <p className="text-sm text-[var(--text-secondary)]">
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--purple)] font-bold mb-2">OutputBuffer / ResizeHandler</h4>
            <p className="text-sm text-[var(--text-secondary)]">
      ...
      <Layer title="PTY 生命周期" icon="🔄" defaultOpen={true}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          <div className="bg-[var(--bg-card)] p-3 rounded-lg border border-[var(--terminal-green)]/30 text-center">
            <div className="text-[var(--terminal-green)] font-bold text-sm mb-1">1. 创建</div>
            <p className="text-xs text-[var(--text-muted)]">
              Agent 调用 run_in_terminal，PtyManager 分配 sessionId 并 spawn PTY
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-3 rounded-lg border border-[var(--cyber-blue)]/30 text-center">
            <div className="text-[var(--cyber-blue)] font-bold text-sm mb-1">2. 连接</div>
            <p className="text-xs text-[var(--text-muted)]">
              Master/Slave fd 对建立，Shell 进程 fork + setsid + exec
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-3 rounded-lg border border-[var(--amber)]/30 text-center">
            <div className="text-[var(--amber)] font-bold text-sm mb-1">3. 通信</div>
            <p className="text-xs text-[var(--text-muted)]">
              双向读写，支持输入写入、输出读取和窗口大小调整
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-3 rounded-lg border border-[var(--purple)]/30 text-center">
            <div className="text-[var(--purple)] font-bold text-sm mb-1">4. 调整</div>
            <p className="text-xs text-[var(--text-muted)]">
              动态 resize 发送 SIGWINCH，子进程重新渲染
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-3 rounded-lg border border-red-500/30 text-center">
      ...
      <Layer title="双向通信协议" icon="🔀" defaultOpen={true}>
        ...
        <div className="mt-4 bg-[var(--bg-terminal)] p-4 rounded-lg">
          <h4 className="text-[var(--terminal-green)] font-bold mb-2">通信协议要点</h4>
          <ul className="text-sm text-[var(--text-secondary)] space-y-2">
            <li><strong className="text-[var(--cyber-blue)]">工具调用驱动：</strong>所有 PTY 操作通过 Tool Scheduler 调度，Model 通过工具调用与 PTY 交互，无直接连接</li>
            <li><strong className="text-[var(--terminal-green)]">异步输出收集：</strong>写入后等待指定时间 (waitForOutput) 收集输出，避免过早读取导致结果不完整</li>
            <li><strong className="text-[var(--amber)]">会话 ID 绑定：</strong>每个 PTY 会话有唯一 sessionId，后续操作 (write/resize/close) 通过 ID 定位目标会话</li>
            <li><strong className="text-[var(--purple)]">状态透传：</strong>每次工具结果都包含 isAlive 标志，Model 可感知会话是否仍然活跃</li>
          </ul>
        </div>
      </Layer>
      ...
      <Layer title="Shell 自动补全交互" icon="⌨️" defaultOpen={false}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--terminal-green)] font-bold mb-2">工作原理</h4>
            <p className="text-sm text-[var(--text-secondary)]">
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <h4 className="text-[var(--cyber-blue)] font-bold mb-2">适用场景</h4>
            <ul className="text-sm text-[var(--text-secondary)] space-y-1">
              <li>- 探索未知环境中的可用命令</li>
              <li>- 自动补全文件路径和目录名</li>
      ...
      <Layer title="核心代码实现" icon="💻" defaultOpen={false}>
        <div className="space-y-6">
          <div>
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-3">PtyManager - 会话管理器</h3>
            <CodeBlock code={ptyManagerCode} language="typescript" title="PtyManager" />
          </div>

          <div>
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-3">TerminalSession - 会话封装</h3>
            <CodeBlock code={terminalSessionCode} language="typescript" title="TerminalSession" />
          </div>

          <div>
            <h3 className="text-lg font-bold text-[var(--text-primary)] mb-3">InputHandler - 输入处理</h3>
      ...
      <Layer title="安全考虑" icon="🛡️" defaultOpen={true}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-red-500/30">
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--amber)]/30">
              <h4 className="text-[var(--amber)] font-bold mb-2">防御策略</h4>
              <ul className="text-sm text-[var(--text-secondary)] space-y-1">
                <li>- 命令白名单过滤</li>
                <li>- 危险模式正则匹配</li>
                <li>- 资源使用量限制</li>
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--terminal-green)]/30">
              <h4 className="text-[var(--terminal-green)] font-bold mb-2">审计与监控</h4>
              <ul className="text-sm text-[var(--text-secondary)] space-y-1">
                <li>- 会话创建/关闭日志</li>
                <li>- 输入/输出记录</li>
                <li>- 异常退出告警</li>
      ...
      <Layer title="与现有系统的集成" icon="🔗" defaultOpen={false}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--cyber-blue)]/30">
              <h4 className="text-[var(--cyber-blue)] font-bold mb-2">Tool Scheduler 集成</h4>
...
toolRegistry.register([
  ...
  closeTerminalTool,
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--terminal-green)]/30">
              <h4 className="text-[var(--terminal-green)] font-bold mb-2">Shell 执行服务协作</h4>
              <p className="text-sm text-[var(--text-secondary)] mb-2">
...
"vim config.ts", "top", "python3"`} />
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--amber)]/30">
              <h4 className="text-[var(--amber)] font-bold mb-2">审批模式 (Approval Mode)</h4>
              <p className="text-sm text-[var(--text-secondary)]">
            ...
            <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--purple)]/30">
              <h4 className="text-[var(--purple)] font-bold mb-2">Agent 循环集成</h4>
              <p className="text-sm text-[var(--text-secondary)]">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-[var(--border-subtle)]">
                    <th className="text-left py-2 text-[var(--text-muted)]">场景</th>
                    <th className="text-left py-2 text-[var(--text-muted)]">推荐方式</th>
                    <th className="text-left py-2 text-[var(--text-muted)]">原因</th>
                  </tr>
                </thead>
                <tbody className="text-[var(--text-secondary)]">
                  <tr className="border-b border-[var(--border-subtle)]/30">
                    <td className="py-2">读取文件内容</td>
                    <td className="py-2 text-[var(--terminal-green)]">run_shell_command</td>
                    <td className="py-2">无需交互，结果确定</td>
                  </tr>
                  <tr className="border-b border-[var(--border-subtle)]/30">
                    <td className="py-2">编辑文件 (vim)</td>
                    <td className="py-2 text-[var(--cyber-blue)]">run_in_terminal</td>
