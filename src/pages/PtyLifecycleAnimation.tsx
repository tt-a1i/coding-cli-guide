...
function Introduction({ isExpanded, onToggle }: { isExpanded: boolean; onToggle: () => void }) {
 ...
 <span className="px-2 py-1 bg-orange-900/50 text-heading rounded text-xs">沙箱系统</span>
 </div>
 </div>
 </div>
 )}
...
export default function PtyLifecycleAnimation() {
 ...
 className="px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-elevated rounded"
 >
 模拟中止 (Ctrl+C)
 </button>
 <button
 ...
 <span className={executionMethod ? 'text-green-400' : 'text-dim'}>
 {executionMethod || '-'}
 </span>
 </div>
 <div className="flex justify-between">
 ...
 <span className={sniffedBytes >= CONSTANTS.MAX_SNIFF_SIZE ? 'text-yellow-400' : 'text-heading'}>
 ...
 chunk.isBinary ? 'bg-red-900/50' : ' bg-elevated'
 }`}
 >
 ...
 line.content.includes('BINARY') ? 'text-red-400' : 'text-heading'
 }`}
 >
 {line.content}
 </div>
 ...
 <div><span className="text-heading">SIGKILL_TIMEOUT_MS</span> = <span className="text-yellow-400">{CONSTANTS.SIGKILL_TIMEOUT_MS}</span></div>
 <div><span className="text-heading">MAX_SNIFF_SIZE</span> = <span className="text-yellow-400">{CONSTANTS.MAX_SNIFF_SIZE}</span></div>
 <div><span className="text-heading">renderTimeout</span> = <span className="text-yellow-400">{CONSTANTS.RENDER_THROTTLE_MS}</span> <span className="text-dim">// ≈60fps</span></div>
 </div>
 </div>
 </div>
