        ...
        <div className="px-6 pb-6 space-y-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              ...
              <div className="text-xs text-[var(--text-muted)]">默认 Token 预算</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--terminal-green)]">Flash</div>
              <div className="text-xs text-[var(--text-muted)]">摘要模型</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--amber)]">3</div>
              <div className="text-xs text-[var(--text-muted)]">摘要规则</div>
            </div>
            <div className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
              <div className="text-2xl font-bold text-[var(--purple)]">2</div>
              <div className="text-xs text-[var(--text-muted)]">Summarizer 类型</div>
            </div>
          </div>

          <div>
            <h4 className="text-sm font-semibold text-[var(--text-muted)] mb-2">摘要决策流程</h4>
...
export async function summarizeToolOutput(
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-[var(--text-primary)] mb-2">Summarizer 摘要系统</h1>
      ...
      <Layer title="摘要流程" icon="🔄" defaultOpen={true}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--cyber-blue)] font-bold mb-2">📊 触发条件</div>
          ...
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--terminal-green)] font-bold mb-2">🔧 摘要模型</div>
            <ul className="text-sm text-[var(--text-secondary)] space-y-1">
      ...
      <Layer title="摘要 Prompt" icon="💬" defaultOpen={true}>
        ...
        <div className="mt-4 p-4 bg-[var(--bg-terminal)]/50 rounded-lg border border-[var(--border-subtle)]">
          <div className="text-sm">
            <strong className="text-[var(--text-primary)]">📝 三种摘要规则：</strong>
            <div className="mt-2 space-y-3">
              <div className="flex items-start gap-2">
                <span className="text-[var(--cyber-blue)]">1.</span>
                <div>
                  <span className="text-[var(--text-primary)]">目录/结构化输出：</span>
                  <span className="text-[var(--text-muted)]">根据对话上下文提取需要的信息</span>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-[var(--terminal-green)]">2.</span>
                <div>
                  <span className="text-[var(--text-primary)]">文本内容：</span>
                  <span className="text-[var(--text-muted)]">直接摘要核心内容</span>
                </div>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-[var(--amber)]">3.</span>
                <div>
                  <span className="text-[var(--text-primary)]">Shell 命令输出：</span>
                  <span className="text-[var(--text-muted)]">保留完整错误堆栈 (&lt;error&gt;) 和警告 (&lt;warning&gt;)</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <Layer title="使用场景" icon="🎯" defaultOpen={false}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--cyber-blue)] font-bold mb-2">📁 文件读取</div>
            <p className="text-sm text-[var(--text-secondary)]">
              读取大文件时，摘要提取关键代码片段和结构信息
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--terminal-green)] font-bold mb-2">🔍 搜索结果</div>
            <p className="text-sm text-[var(--text-secondary)]">
              Grep 返回大量匹配时，摘要为最相关的结果列表
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--amber)] font-bold mb-2">💻 Shell 输出</div>
            <p className="text-sm text-[var(--text-secondary)]">
              npm install 等长输出，摘要结果但保留错误堆栈
            </p>
          </div>
          <div className="bg-[var(--bg-card)] p-4 rounded-lg border border-[var(--border-subtle)]">
            <div className="text-[var(--purple)] font-bold mb-2">📊 目录列表</div>
            <p className="text-sm text-[var(--text-secondary)]">
