...
export function SlashCommands() {
 ...
 <strong>斜杠命令</strong>
 <p className="text-xs text-body">CLI 元控制命令</p>
 </div>
 <div className="text-center">
 <div className="text-2xl mb-1 font-mono">@</div>
 <strong>At 命令</strong>
 <p className="text-xs text-body">文件/目录注入</p>
 </div>
 <div className="text-center">
 <div className="text-2xl mb-1 font-mono">!</div>
 <strong>Shell 命令</strong>
 <p className="text-xs text-body">Shell 执行/模式切换</p>
 </div>
 </div>
 </HighlightBox>
 ...
 <CommandCard
 name="/chat share [file]"
 desc="导出会话到 Markdown/JSON"
 />
 <CommandCard
 name="/summary"
 desc="生成项目摘要到 .gemini/PROJECT_SUMMARY.md"
 />
 <CommandCard name="/compress" desc="手动压缩聊天历史，节省 Token" />
 <CommandCard
 name="/stats"
 desc="显示会话统计（Token 使用、时长等）"
 />
 ...
 <CommandCard
 name="/policies list"
 desc="列出当前生效的 Policy 规则"
 />
 <CommandCard
 name="/permissions trust [path]"
 desc="管理文件夹信任（未提供 path 时使用当前目录）"
 />
 <CommandCard
 name="/skills list [nodesc] [all]"
 desc="列出可用技能（可显示内建技能）"
 />
 ...
 {/* 帮助与信息 */}
 <h4 className="text-lg text-heading font-bold mb-3">ℹ️ 帮助与信息</h4>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-6">
 ...
 <p className="text-sm text-body mb-2">执行一条命令后返回 CLI</p>
 <code className="text-xs text-body block">!git status</code>
 <code className="text-xs text-body block">!npm test</code>
 </div>
 ...
 <code className="text-xs text-body block">
 UI 显示 Shell Mode 指示器
 </code>
 <code className="text-xs text-body block">
 所有输入直接作为 shell 命令执行
 </code>
...
class CommandService {
 ...
 gemini-cli/packages/cli/src/services/CommandService.ts
 </code>
 <span className="text-body">命令服务（加载、冲突处理）</span>
 </div>
 <div className="flex items-center gap-2">
 <code className="bg-base/30 px-2 py-1 rounded">
 gemini-cli/packages/cli/src/services/BuiltinCommandLoader.ts
 </code>
 <span className="text-body">内置命令加载器</span>
 </div>
 <div className="flex items-center gap-2">
 <code className="bg-base/30 px-2 py-1 rounded">
 gemini-cli/packages/cli/src/services/FileCommandLoader.ts
 </code>
 <span className="text-body">自定义命令加载器</span>
 </div>
 <div className="flex items-center gap-2">
 <code className="bg-base/30 px-2 py-1 rounded">
 gemini-cli/packages/cli/src/ui/commands/types.ts
 </code>
 <span className="text-body">SlashCommand 接口定义</span>
 </div>
 <div className="flex items-center gap-2">
 <code className="bg-base/30 px-2 py-1 rounded">
 gemini-cli/docs/cli/commands.md
 </code>
 <span className="text-body">官方命令文档</span>
 </div>
 </div>
 </Layer>
...
`}
 ...
 <tr>
 <th className="px-3 py-2 text-left text-body">模式</th>
 <th className="px-3 py-2 text-left text-body">
 Token 消耗
 </th>
 <th className="px-3 py-2 text-left text-body">延迟</th>
 <th className="px-3 py-2 text-left text-body">
 典型场景
 </th>
 </tr>
 ...
 <tr>
 <td className="px-3 py-2 text-heading">AI_PROMPT</td>
 <td className="px-3 py-2 text-body">500 - 5000</td>
 <td className="px-3 py-2 text-body">1-10s</td>
 <td className="px-3 py-2 text-dim">生成、分析、总结</td>
 </tr>
 <tr>
 <td className="px-3 py-2 text-heading">HYBRID</td>
 <td className="px-3 py-2 text-body">200 - 2000</td>
 <td className="px-3 py-2 text-body">500ms - 5s</td>
 <td className="px-3 py-2 text-dim">
 压缩、优化、上下文处理
 </td>
 </tr>
 ...
 <tr>
 <td className="px-3 py-2 text-body">退出 CLI</td>
 <td className="px-3 py-2 text-heading font-mono">
 /quit 或 /exit
 </td>
 <td className="px-3 py-2 text-dim">
 ...
 <tr>
 <td className="px-3 py-2 text-body">退出前保存/总结</td>
 <td className="px-3 py-2 text-heading font-mono">
 Ctrl+C Ctrl+C
 </td>
 <td className="px-3 py-2 text-dim">
 ...
 <tr>
 <td className="px-3 py-2 text-body">切换审批模式</td>
 <td className="px-3 py-2 text-heading font-mono">
 Shift+Tab / Ctrl+Y
 </td>
 <td className="px-3 py-2 text-dim">
 Shift+Tab：default ↔ autoEdit；Ctrl+Y：default ↔ yolo
 </td>
 </tr>
 </tbody>
 </table>
 ...
 <strong>确定性操作</strong>：/stats、/clear、/quit
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>节省 Token</strong>：本地命令不消耗 API
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>配置更改</strong>：/theme、/vim、/model
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>快速操作</strong>：不需要等待 AI 响应
 </span>
 </li>
 </ul>
 </div>
 ...
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>复杂任务</strong>："帮我重构这个函数"
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>需要理解</strong>："为什么这个测试失败？"
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>创造性工作</strong>："写一个新的 API 端点"
 </span>
 </li>
 <li className="flex items-start gap-2">
 <span className="text-heading">→</span>
 <span className="text-body">
 <strong>上下文相关</strong>：需要理解之前的对话
 </span>
 </li>
 </ul>
 </div>
 ...
 <div className="bg-surface rounded p-3 mt-2 space-y-2">
 <code className="block text-heading text-sm">!npm test</code>
 <code className="block text-heading text-sm">
 帮我修复这些测试失败
 </code>
 <p className="text-xs text-dim">
...
4. 验证部署成功`}
 ...
 <li>
 <strong>文件名</strong>即为命令名（不含 .md 后缀）
 </li>
 <li>
 <strong>Front Matter</strong>（可选）：description 字段显示在
 /help 中
 </li>
 <li>
 <strong>正文</strong>：发送给 AI 的提示内容
 </li>
 <li>
 支持<strong>变量替换</strong>：使用 {'{{arg}}'} 接收参数
 </li>
 </ul>
 </HighlightBox>
...
`}
 ...
 同名命令时，后加载的会覆盖先加载的。MCP 命令冲突时会重命名为{' '}
 <code className="bg-base/30 px-1 rounded">extension.command</code>
 。
 </p>
 </div>
 </Layer>
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge">
 <h4 className="text-heading font-semibold mb-2">
 Q: /compress 和 /chat save 有什么区别？
 </h4>
 <p className="text-body text-sm">
 <strong>/compress</strong> 会让 AI 总结并压缩历史，减少 Token
 但可能丢失细节。
 <strong>/chat save</strong> 完整保存当前状态，可以精确恢复但不减少
 Token。
 </p>
 </div>
 ...
 <p className="text-body text-sm">
 <strong>!command</strong> 你自己直接执行，结果不发给 AI，不消耗
 Token。 让 AI 执行会消耗 Token，但 AI 可以根据结果继续工作。
 </p>
 </div>
 ...
 <p className="text-body text-sm">
 输入 <code className="bg-base/30 px-1 rounded">/help</code> 或{' '}
 <code className="bg-base/30 px-1 rounded">/?</code>{' '}
 查看内置命令。 输入{' '}
 <code className="bg-base/30 px-1 rounded">/</code> 后按 Tab
 可以自动补全。
 </p>
 </div>
 </div>
 ...
 id: 'at-cmd',
 label: '@ 命令详解',
 description: '文件和目录注入的完整用法',
 },
 { id: 'config', label: '配置系统', description: '自定义命令和设置' },
 {
 id: 'extension',
 label: '扩展系统',
 description: 'MCP 命令和扩展开发',
 },
 {
...
function ShortcutCard({ keys, desc }: { keys: string; desc: string }) {
 ...
 <span className="text-sm text-body">{desc}</span>
 </div>
 );
}
