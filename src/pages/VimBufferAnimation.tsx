...
function TextVisualizer({
 ...
 return (
 <div className="bg-base/40 rounded-lg p-4 font-mono">
 ...
 {/* 导航指示 */}
 {navigation && (
 <div className="mt-4 p-3 bg-elevated/10 rounded border border-edge/30">
...
function ScriptLegend({ chars }: { chars: CharInfo[] }) {
 ...
 <div
 key={script}
 className="flex items-center gap-2 p-2 bg-base/30 rounded"
 >
 <span
...
function VimCommandReference() {
 ...
 ${cmd.type === 'word'
 ? ' bg-elevated/10 border border-edge/30'
 : ' bg-elevated/10 border border-edge/30'
 }
 `}
 >
 ...
 <span className="text-heading">●</span> 词命令: 识别脚本边界
 <br />
 <span className="text-heading">●</span> 大词命令: 只识别空白
 </div>
 </div>
 );
}
