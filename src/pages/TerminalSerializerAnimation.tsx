...
function TerminalGridVisualizer({
 ...
 ${isCurrent ? ' bg-elevated text-heading' : 'text-body'}
 `}
 >
 {char}
 </span>
...
function BitFlagsVisualizer({
 ...
 <span className="font-mono text-xs text-dim">{e.mask}</span>
 ...
 {result && (
 <div className="mt-3 pt-3 border-t border-edge text-center">
 <span className="text-dim">结果: </span>
 <span className="font-mono text-heading text-lg">{result}</span>
 </div>
 )}
 </div>
 );
...
function ColorModeVisualizer({
 ...
 <div
 key={i}
 className="flex items-center gap-3 p-2 rounded bg-base/20"
 >
 <span className="w-20 text-sm text-body">{m.mode}</span>
 ...
 <span className="text-sm text-heading">{m.result}</span>
 </div>
 </div>
 ))}
 </div>
...
function CellGroupVisualizer({
 ...
 <div className="text-xs text-dim">{group.attrs}</div>
 </div>
 ))}
 </div>
 )}
...
function TokenAggregateVisualizer({
 ...
 {input && (
 <div className="mb-4">
 <div className="text-xs text-dim mb-1">输入: {input.length} 单元格</div>
 <div className="flex gap-0.5">
 {input.map((char, i) => (
 <span
 key={i}
 className="w-5 h-6 flex items-center justify-center bg-base/30 text-body text-xs font-mono"
 >
 {char}
 </span>
 ))}
 ...
 {/* 输出 */}
 <div>
 <div className="text-xs text-dim mb-1">输出: {output.length} Token</div>
 <div className="flex gap-2">
 {output.map((token, i) => (
 <div
 key={i}
...
function OutputStructureVisualizer({
 ...
 <span className="text-xs text-dim w-8">L{y}</span>
...
function StatsVisualizer({
 ...
 <div className="text-xs text-dim">压缩率</div>
 </div>
 </div>
 </div>
 );
...
export function TerminalSerializerAnimation() {
 ...
 {step.highlight && (
 <div className="inline-block px-3 py-1 rounded-full text-sm font-medium bg-elevated/20 text-heading">
 {step.highlight}
 </div>
 )}
 </div>
