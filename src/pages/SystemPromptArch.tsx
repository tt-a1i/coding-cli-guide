...
function Introduction({ isExpanded, onToggle }: { isExpanded: boolean; onToggle: () => void }) {
 return (
 <div className="mb-8 bg-surface rounded-lg border border-edge/30 overflow-hidden">
 <button
 onClick={onToggle}
...
export function SystemPromptArch() {
 ...
 <div className="flex-shrink-0 w-8 h-8 rounded-full bg-elevated/20 flex items-center justify-center text-heading font-bold">1</div>
 <div className="flex-1 bg-elevated/10 rounded-lg p-3">
    <div className="font-semibold text-heading">加载基础 Prompt (Base Prompt)</div>
 <p className="text-sm text-body">检查 GEMINI_SYSTEM_MD 环境变量，决定使用默认内置 Prompt 还是从文件加载</p>
 </div>
 </div>
 ...
 <div className="flex items-center gap-4">
 <div className="flex-shrink-0 w-8 h-8 rounded-full bg-elevated flex items-center justify-center text-heading font-bold">3</div>
 <div className="flex-1 bg-elevated rounded-lg p-3">
    <div className="font-semibold text-heading">工具调用示例注入 (Tool Examples)</div>
 <p className="text-sm text-body">根据目标模型选择适配的工具调用格式示例</p>
 </div>
 </div>
...
\`;
 ...
 <div className="bg-surface rounded-lg p-4 border border-edge">
 <h4 className="font-semibold text-heading mb-2">General (OpenAI 兼容)</h4>
 ...
 <p className="text-xs text-dim mt-2">匹配 /gemini.*-coder/i</p>
 </div>

 <div className="bg-surface rounded-lg p-4 border border-edge">
 <h4 className="font-semibold text-heading mb-2">Gemini Vision 格式</h4>
 <div className="text-xs font-mono bg-base/40 p-2 rounded text-body">
 {'<tool_call>'}<br/>
 {'{"name": "run_shell_command", "arguments": {"command": "npm run build"}}'}<br/>
 {'</tool_call>'}
 </div>
 <p className="text-xs text-dim mt-2">匹配 /gemini.*-vl/i</p>
 </div>
 </div>
...
// [User Memory from GEMINI.md]`}
 ...
 - 偏好函数式编程风格<br/>
 <br/>
 <div className="text-heading"># 项目规范</div>
...
- Skill Guidance: treat <INSTRUCTIONS> as expert rules`}
 ...
 <span className="text-heading">getCoreSystemPrompt(config, userMemory)</span>
 </div>
 ...
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">├──</span>
 <span className="text-body">basePrompt 内部包含动态 IIFE:</span>
 </div>
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">│ ├──</span>
 <span className="text-dim">Sandbox 检测注入</span>
 </div>
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">│ ├──</span>
 <span className="text-dim">Git 仓库检测注入</span>
 </div>
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">│ └──</span>
 ...
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">├──</span>
 <span className="text-body">追加 userMemory (GEMINI.md)</span>
 </div>
 <div className="pl-4 flex items-center gap-2">
 <span className="text-heading">└──</span>
 <span className="text-body">返回完整 System Prompt</span>
 </div>
 </div>
 </div>
 </Layer>
 ...
 <li className="flex items-start gap-2">
 <code className="text-heading">GEMINI_WRITE_SYSTEM_MD</code>
 <span>导出当前 Prompt 到文件</span>
 </li>
 <li className="flex items-start gap-2">
 <code className="text-heading">GEMINI_PROMPT_*</code>
 <span>按段开关 Prompt 片段（可选）</span>
 </li>
 <li className="flex items-start gap-2">
 <code className="text-heading">SANDBOX</code>
 <span>沙箱模式指示器</span>
 </li>
 </ul>
 </div>
