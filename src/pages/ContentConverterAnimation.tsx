...
function FormatComparisonVisualizer({
 ...
 {isGeminiToOpenai && (
 <span className="px-1.5 py-0.5 rounded text-xs bg-elevated/20 text-heading">
 源
 </span>
 )}
 </div>
...
function CleanupVisualizer({
 ...
 <div
 key={i}
 className="p-2 rounded bg-base/30 text-xs font-mono"
 >
 ...
 <div>
 <div className="text-xs text-dim mb-2">清理后</div>
 <div className="space-y-1">
 {after.map((msg, i) => (
 <div
 key={i}
...
function MergeVisualizer({
 ...
 <div>
 <div className="text-xs text-dim mb-2">合并后 ({after.length} 条)</div>
 <div className="space-y-1">
 {after.map((msg, i) => (
 <div
 key={i}
...
export function ContentConverterAnimation() {
 ...
 <div
 key={i}
 className="p-3 rounded-lg bg-base/30 border border-edge"
 >
 <div className="font-medium text-heading text-sm">{f.name}</div>
