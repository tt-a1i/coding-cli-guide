...
function Introduction({ isExpanded, onToggle }: { isExpanded: boolean; onToggle: () => void }) {
 ...
 <div className="text-heading font-semibold text-sm">Layer 2: 内容流</div>
 <div className="text-xs text-dim mt-1">
 滑动窗口检测重复内容块<br/>
 阈值: 10 次相同块
 </div>
 ...
 <div className="bg-surface p-3 rounded border border-edge">
 <div className="text-heading font-semibold text-sm">Layer 3: LLM 分析</div>
 <div className="text-xs text-dim mt-1">
 AI 自我评估是否陷入循环<br/>
 每 3-15 turns 自适应检查
 </div>
...
function LayerVisualizer({
 ...
 />
 <span className="font-bold text-heading">{layer.name}</span>
 </div>
 <div className="text-xs text-body">阈值: {layer.threshold}</div>
 <div
...
function HashVisualizer({ data }: { data?: { toolCall?: unknown; hash?: string } }) {
 ...
 <div className="text-body text-xs mb-1">Input</div>
 <pre className="text-heading overflow-x-auto">
 {JSON.stringify(data.toolCall, null, 2)}
 </pre>
 </div>
 <svg className="w-8 h-8 text-dim flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" />
 </svg>
 <div className="flex-1 p-3 rounded bg-base/30 font-mono text-sm">
 <div className="text-body text-xs mb-1">SHA256 (16 chars)</div>
 <div className="text-heading text-lg tracking-wider">{data.hash}</div>
 </div>
 </div>
 </div>
 );
...
function RepeatVisualizer({
 ...
 />
 <span className="text-xs text-body font-mono">{item.count}</span>
 </div>
 ))}
 {/* 阈值线 */}
 <div
...
function WindowVisualizer({ data }: { data?: { content: string; windowSize: number; currentPosition: number } }) {
 ...
 <span className="bg-elevated/30 text-heading px-1 rounded">
 {window}
 </span>
...
function DistanceVisualizer({
 ...
 <span className="text-xs text-dim mt-1">{pos}</span>
 </div>
...
function LLMAnalysisVisualizer({
 ...
 <span className="text-sm font-bold text-heading">LLM 分析结果</span>
 <div className="flex items-center gap-2">
 ...
 <div className="text-sm text-body">{result.reason}</div>
 </div>
 );
}
...
export function LoopDetectionAnimation() {
 ...
 />
 <span className="font-bold text-heading">{layer.name}</span>
 <span className="text-xs text-dim ml-auto">{layer.complexity}</span>
 </div>
 <p className="text-xs text-body">{layer.desc}</p>
 </div>
 ))}
 </div>
 </div>
