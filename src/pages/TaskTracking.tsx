      ...
      {isExpanded && (
        <div className="px-6 pb-6 space-y-5">
          ...
          <div className="grid grid-cols-4 gap-3">
            {[
              { val: '4', label: 'CRUD 工具', color: 'var(--purple)' },
              { val: '4', label: '任务状态', color: 'var(--terminal-green)' },
              { val: '3', label: '核心组件', color: 'var(--cyber-blue)' },
              { val: 'JSON', label: '持久化格式', color: 'var(--amber)' },
            ].map(({ val, label, color }) => (
              <div key={label} className="bg-[var(--bg-card)] rounded-lg p-3 text-center border border-[var(--border-subtle)]">
          ...
          <div>
            <h4 className="text-sm font-semibold text-[var(--text-muted)] mb-2">任务生命周期</h4>
            <div className="flex items-center gap-2 flex-wrap text-sm">
              {[
                { text: 'createTask', color: 'var(--cyber-blue)' },
                { text: 'pending', color: 'var(--amber)' },
                { text: 'in_progress', color: 'var(--purple)' },
                { text: 'completed', color: 'var(--terminal-green)' },
              ].flatMap(({ text, color }, i) => [
                ...(i > 0 ? [<span key={`a${i}`} className="text-[var(--text-muted)]">→</span>] : []),
...
export class TaskPersistence {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* 概述 */}
      <section id="overview">
        <h2 className="text-2xl font-bold text-cyan-400 mb-4">任务追踪系统</h2>
      ...
      <section id="architecture">
        <h3 className="text-xl font-semibold text-cyan-400 mb-4">架构设计</h3>
        <MermaidDiagram chart={architectureChart} title="TaskTracker 系统架构" />

        <div className="mt-4 bg-gray-800/50 rounded-lg p-4">
          <h4 className="font-semibold text-cyan-400 mb-3">核心组件</h4>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-300">
            <div>
              <p className="font-semibold text-[var(--purple)] mb-2">TaskTracker</p>
              <p>核心协调器，管理任务生命周期，处理状态转换验证和依赖关系检查。</p>
            </div>
            <div>
              <p className="font-semibold text-[var(--amber)] mb-2">TaskStore</p>
              <p>基于 Map 的内存存储加 JSON 文件持久化，提供高效的 CRUD 操作。</p>
            </div>
            <div>
              <p className="font-semibold text-[var(--terminal-green)] mb-2">TaskVisualization</p>
      ...
      <section id="state-machine">
        <h3 className="text-xl font-semibold text-cyan-400 mb-4">任务状态机</h3>
        <MermaidDiagram chart={stateMachineChart} title="任务状态流转" />

        <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { icon: '○', name: 'pending', desc: '等待开始', color: 'var(--amber)' },
            { icon: '◐', name: 'in_progress', desc: '执行中', color: 'var(--purple)' },
            { icon: '●', name: 'completed', desc: '已完成', color: 'var(--terminal-green)' },
            { icon: '✕', name: 'blocked', desc: '被依赖阻塞', color: '#f87171' },
          ].map(({ icon, name, desc, color }) => (
            <div key={name} className="bg-[var(--bg-card)] rounded-lg p-3 border" style={{ borderColor: `color-mix(in srgb, ${color} 30%, transparent)` }}>
      ...
      <Layer title="工具接口" icon="🔧">
        <div className="space-y-4">
          ...
          <h4 className="text-sm font-semibold text-[var(--text-muted)]">listTasks 响应示例</h4>
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="bg-gray-800/50">
                  {['工具', '操作', '必需参数', '说明'].map(h => (
                    <th key={h} className="border border-gray-700 p-3 text-left text-gray-400">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="text-gray-300">
                {[
                  ...
                  <tr key={tool} className={i % 2 ? 'bg-gray-800/30' : ''}>
                    <td className="border border-gray-700 p-3"><code className="text-cyan-300">{tool}</code></td>
                    <td className="border border-gray-700 p-3">{op}</td>
                    <td className="border border-gray-700 p-3"><code>{param}</code></td>
                    <td className="border border-gray-700 p-3">{desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Layer title="可视化展示" icon="📊">
        <div className="space-y-4">
          <div className="bg-[var(--bg-terminal)] rounded-lg p-4 font-mono text-xs overflow-x-auto">
            <div className="text-[var(--text-muted)] mb-2"># 任务列表示例输出</div>
            <pre className="text-[var(--terminal-green)]">{`┌──────────┬────────────────────────┬────────────┬────────┐
...
Progress: [████████████████░░░░░░░░░░░░░░] 1/4 (25%)`}</pre>
      ...
      <section id="plan-mode-integration">
        <h3 className="text-xl font-semibold text-cyan-400 mb-4">与 Plan Mode 的协作</h3>
      ...
      <Layer title="设计决策" icon="💡">
        <div className="space-y-4">
          {[
            ...
            <div key={q} className="bg-[var(--bg-terminal)]/50 rounded-lg p-4 border-l-4" style={{ borderColor: color }}>
        ...
        <div className="grid grid-cols-1 gap-2 text-sm">
          {[
            ...
            <div key={path} className="flex items-start gap-2">
              <code className="bg-black/30 px-2 py-1 rounded text-xs whitespace-nowrap">{path}</code>
