...
export default function OAuthDeviceFlowAnimation() {
 ...
 <div>
 <h1 className="text-2xl font-bold text-heading font-mono">OAuth 登录流程（上游 gemini-cli）</h1>
 ...
 <div className="text-sm font-mono text-heading">Env Signals（模拟）</div>
 ...
 t.v
 ? ' bg-elevated/10 border-edge/40 text-heading'
 : 'bg-base border-edge text-body hover:text-heading'
 }`}
 disabled={isPlaying}
 ...
 <div className="text-sm font-mono text-heading">Flow Snapshot</div>

 <div className="space-y-2 text-sm">
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">redirectUri</span>
 <span className="font-mono text-heading truncate">{redirectUri ?? '-'}</span>
 </div>
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">state</span>
 <span className="font-mono text-heading truncate">{state ? shortToken(state) : '-'}</span>
 </div>
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">authUrl</span>
 <span className="font-mono text-heading truncate">{authUrl ? shortToken(authUrl) : '-'}</span>
 </div>
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">authCode</span>
 <span className="font-mono text-heading truncate">{authCode ?? '-'}</span>
 </div>
 </div>
 ...
 className="w-full mt-2 px-3 py-2 rounded-lg border border-edge/30 bg-elevated/10 text-heading font-mono text-sm disabled:opacity-50 disabled:cursor-not-allowed"
 ...
 <div className="bg-surface border border-edge rounded-lg p-4">
 <div className="text-sm font-mono text-heading mb-3">Tokens</div>
 {tokens ? (
 <div className="space-y-2 text-sm font-mono">
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">access_token</span>
 <span className="text-heading">{shortToken(tokens.accessToken)}</span>
 </div>
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">refresh_token</span>
 <span className="text-heading">{tokens.refreshToken ? shortToken(tokens.refreshToken) : '-'}</span>
 </div>
 <div className="flex items-center justify-between gap-2">
 <span className="text-body">expiry_date</span>
 <span className="text-heading">{new Date(tokens.expiryDateMs).toLocaleString()}</span>
 </div>
 </div>
 ) : (
 <div className="text-sm text-dim font-mono">尚未获取 tokens</div>
 )}
 </div>

 <div className="bg-surface border border-edge rounded-lg p-4">
 <div className="text-sm font-mono text-heading mb-3">Logs</div>
 <div className="bg-base border border-edge rounded p-3 max-h-[240px] overflow-y-auto">
 {logs.length === 0 ? (
 <div className="text-sm text-dim font-mono">点击“开始”运行动画</div>
 ) : (
 <pre className="text-xs text-body font-mono whitespace-pre-wrap m-0">
 {logs.join('\n')}
 </pre>
 )}
 </div>
 ...
 <div className="bg-surface border border-edge rounded-lg p-4">
 <div className="text-sm font-mono text-dim mb-2">Source anchors（上游）</div>
 <ul className="text-xs text-body font-mono space-y-1 m-0 pl-5 list-disc">
