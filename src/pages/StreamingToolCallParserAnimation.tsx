...
function BufferVisualizer({
 ...
 ${isActive
 ? ' border-edge shadow-lg shadow-sky-400/20'
 : ' border-edge'
 }
 `}
 style={{
 ...
 <span className="text-xs font-mono text-body">Buffer</span>
 <span
 ...
 <span className="text-xs font-mono text-dim">{buffer.toolId}</span>
 </div>
...
function StreamVisualizer({ currentPhase }: { currentPhase: ParserPhase }) {
 ...
 : ' bg-surface text-dim border border-edge'
 }
 ${i === activeIndex ? 'ring-2 ring-edge animate-pulse' : ''}
 `}
 >
 {chunk}
 </span>
...
function CollisionVisualizer({ show }: { show: boolean }) {
 ...
 <div className="font-mono text-heading">call_def456 → 分配新索引 1</div>
 </div>
 </div>
 </div>
 </div>
...
export function StreamingToolCallParserAnimation() {
 ...
 不存在 OpenAI 的 <code className="bg-base/30 px-1 rounded">tool_calls</code> chunk 解析流程。
 </div>
 </div>
 ...
 {/* Buffer 状态 */}
 <div>
 <h3 className="text-sm font-bold text-body mb-3 font-mono">Buffer 状态</h3>
 ...
 <div>
 <div className="flex items-center justify-between mb-3">
 <h3 className="text-sm font-bold text-body font-mono">源码实现</h3>
 <button
 ...
 className="px-4 py-2 rounded-lg bg-surface text-body hover:bg-elevated transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
 >
 上一步
 </button>
 <button
 ...
 />
 <span className="font-medium text-heading">{strategy.name}</span>
 </div>
 <p className="text-xs text-body">{strategy.desc}</p>
 </div>
 ))}
 </div>
 </div>
