...
function CacheVisualizer({
 ...
 {query && (
 <div className="mb-3 p-2 rounded bg-base/30">
 <span className="text-dim text-xs">Query: </span>
 <span className="text-heading font-mono">{query}</span>
 </div>
 )}
...
function PrefixScanVisualizer({
 ...
 >
 <span className="font-mono text-sm text-heading">"{item.key}"</span>
 ...
 <span className="text-xs text-dim">不匹配</span>
 )}
 </div>
 ))}
 </div>
...
function ScopeOptimizationVisualizer({
 ...
 <span key={i} className="px-2 py-0.5 rounded bg-base/30 text-xs text-body font-mono">
 {file}
 </span>
 ))}
 </div>
...
function FilterVisualizer({
 ...
 <span className="text-xs text-dim">结果: </span>
...
function StatsVisualizer({
 ...
 <div className="text-2xl font-bold text-heading">{stats.cacheSize}</div>
 <div className="text-xs text-dim">条目数</div>
 </div>
 </div>
...
function SequenceVisualizer({
 ...
 {sequence.map((item, i) => (
 <div key={i} className="flex items-center gap-3 p-2 rounded bg-base/20">
