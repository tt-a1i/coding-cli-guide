...
export function CustomCommands() {
 ...
 <h4 className="text-lg text-heading font-bold mb-3 mt-5">TOML 文件格式</h4>
 <CodeBlock
 title="命令定义 Schema"
 code={`# 必填字段
prompt = """
...
description = "命令描述（显示在 /help 中）"`}
 ...
 <h4 className="text-lg text-heading font-bold mb-3">状态变化</h4>
 ...
 <h4 className="text-lg text-heading font-bold mb-3 mt-6">Prompt 处理流程</h4>
 <MermaidDiagram chart={promptProcessingFlow} title="处理器链执行顺序" />

 <h4 className="text-lg text-heading font-bold mb-3 mt-6">Shell 注入安全流程</h4>
...
if (!foundEnd) {
 ...
 <h4 className="text-lg text-heading font-bold mb-3 mt-5">处理器选择逻辑</h4>
 <CodeBlock
...
if (!prompt.includes('{{args}}')) {
 ...
 <div>
 <h5 className="text-md text-heading font-semibold mb-2">1. TOML 解析失败</h5>
 ...
 <div>
 <h5 className="text-md text-heading font-semibold mb-2">2. Shell 命令执行失败</h5>
...
fatal: not a git repository (or any of the parent directories): .git`}
 ...
 <div>
 <h5 className="text-md text-heading font-semibold mb-2">3. 文件注入失败</h5>
 ...
 <div>
 <h5 className="text-md text-heading font-semibold mb-2">4. Blocklist 命令</h5>
 <HighlightBox variant="red">
 <p className="text-sm text-body mb-2">
...
if (!allAllowed && isHardDenial) {
 ...
 <div>
 <h5 className="text-md text-heading font-semibold mb-2">5. 用户取消确认</h5>
 ...
 <strong className="text-heading">3. sessionShellAllowlist（会话允许列表）</strong>
 <p className="mt-1">运行时动态维护，用户通过确认对话框批准的命令会添加到此列表，会话期间无需重复确认</p>
 </div>
 </div>
 </HighlightBox>

 <h4 className="text-lg text-heading font-bold mb-3 mt-5">安全配置</h4>
 <CodeBlock
...
mode = "DEFAULT" # DEFAULT | YOLO | AUTO_EDIT`}
 ...
 <h4 className="text-lg text-heading font-bold mb-3 mt-5">三大注入机制详解</h4>

 {/* {{args}} */}
 <div className="mb-6">
 <h5 className="text-md text-heading font-semibold mb-3">1. {'{{args}}'} — 参数替换</h5>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 <div className="bg-elevated/10 border border-edge rounded-lg p-4">
 <h6 className="text-heading font-bold mb-2">原始替换（prompt 主体）</h6>
 <p className="text-sm text-body mb-2">
...
# → "请修复这个问题: 按钮错位"`}
 ...
 <h6 className="text-heading font-bold mb-2">Shell 转义（!{'{...}'} 内部）</h6>
 <p className="text-sm text-body mb-2">
...
"""`}
 ...
 {/* @{...} */}
 <div className="mb-6">
 <h5 className="text-md text-heading font-semibold mb-3">3. @{'{...}'} — 文件注入</h5>
 <p className="text-sm text-body mb-3">
...
"""`}
 ...
 <h4 className="text-lg text-heading font-bold mb-3 mt-5">完整示例</h4>
 <CodeBlock
...
"""`}
 ...
 <div className="bg-base/50 rounded-lg p-4 ">
 <h4 className="text-heading font-bold mb-2">为什么需要三层允许列表机制？</h4>
 <div className="text-sm text-body space-y-2">
 ...
 <div className="bg-base/50 rounded-lg p-4 ">
 <h4 className="text-heading font-bold mb-2">为什么处理器链顺序是 @{} → !{} → args？</h4>
 <div className="text-sm text-body space-y-2">
...
function SourceLink({ path, desc }: { path: string; desc: string }) {
 ...
 <code className="bg-base/30 px-2 py-1 rounded text-xs">{path}</code>
